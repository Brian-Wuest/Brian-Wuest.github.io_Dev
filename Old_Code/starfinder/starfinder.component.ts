...
export class StarfinderComponent implements OnInit {
	...
	createRaceInfo() {
		this.races = [
			new Race("Android", [
				new AttributeModifier("DEX", 2),
				new AttributeModifier("INT", 2),
				new AttributeModifier("CHR", -2)
			]),
			new Race("Human", []),
			new Race("Kasatha", [
				new AttributeModifier("STR", 2),
				new AttributeModifier("WIS", 2),
				new AttributeModifier("INT", -2)
			]),
			new Race("Lashunta (Damaya)", [
				new AttributeModifier("INT", 2),
				new AttributeModifier("CHR", 2),
				new AttributeModifier("CON", -2)
			]),
	...
	getRacialStat(attribute: CharacterAttribute): number {
		if (this.currentRace) {
			if (
				this.getIsHuman() &&
				this.humanModifier &&
				this.humanModifier.name === attribute.name
			) {
				return 2;
	...
	getThemeStat(attribute: CharacterAttribute): number {
		if (this.currentTheme) {
			if (
				this.getIsThemeLess() &&
				this.themelessModifier &&
				this.themelessModifier.name === attribute.name
			) {
				return 1;
			} else if (
				this.currentTheme.modifier &&
				attribute.name === this.currentTheme.modifier.attributeName
			) {
	...
	getComputedStat(attribute: CharacterAttribute): number {
		return (
			10 +
			this.getRacialStat(attribute) +
			this.getThemeStat(attribute) +
			attribute.points
		);
	}
