...
export class StatTrackerComponent implements OnInit {
    ...
    loadCharacters() {
        ...
        if (this.globals.getSupportsHtml5Storage()) {
            if (localStorage.statTrackerCharacters) {
                this.characters = JSON.parse(localStorage.statTrackerCharacters);

                if (this.characters && this.characters.length > 0) {
                    this.updateSelectedCharacter(this.characters[0]);

                    if (this.characters[0].stats && this.characters[0].stats.length > 0) {
    ...
    saveCharacters() {
        if (this.globals.getSupportsHtml5Storage()) {
            localStorage.statTrackerCharacters = JSON.stringify(this.characters);
        }
    }
    ...
    updateSelectedCharacter(character: Character) {
        ...
        if (foundSelectedCharacter) {
            ...
            if (this.selectedCharacter.stats && this.selectedCharacter.stats.length > 0) {
    ...
    removeStat() {
        if (this.selectedCharacter && this.selectedStat) {
            for (let i = 0; i < this.selectedCharacter.stats.length; i++) {
                if (this.selectedStat.id === this.selectedCharacter.stats[i].id) {
